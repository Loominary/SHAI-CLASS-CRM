...
module.exports = {
    addCustomer: async function (req, res, next) {
        ...
        const sql = "INSERT INTO customers(name, phone, email, country_id)" +
                ...
                try{
                    const result = await database.query(sql, [reqBody.name, reqBody.phone, reqBody.email, reqBody.country]); //awaiting the query which includes the connection already. Will return [rows, fields]
                    console.log(result);
    ...
    customersList: async function (req, res) {
        const sql = "SELECT cust.name, cust.phone, cust.email, countries.name AS country FROM `customers` cust JOIN countries ON cust.country_id = countries.id;";
